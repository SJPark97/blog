...
import { convertNotionhqToNotionClient } from './page.utils'
...
export class NotionAPI {
  ...
  constructor({
    ...
    this.notionClient = new Client({ auth: hqAuthToken })
  ...
  public async getPage(
    ...
    // notionhq로 받은 데이터를 react-notion-x에서 사용할 수 있는 형태로 변환
    const pageData = await this.notionClient.pages.retrieve({
      page_id: pageId
    })
    const listData = await this.notionClient.blocks.children.list({
      block_id: pageId
    })
    const recordMap2 = convertNotionhqToNotionClient(pageData, listData)
    // const recordMap = recordMap2 as notion.ExtendedRecordMap
